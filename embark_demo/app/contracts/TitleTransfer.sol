...
contract TitleRegistry is Mortal{
    ...
    struct IndexValue { uint keyIndex; string propertyId; string tttId; string buyer; string seller; }
    ...
    function addTitleTransfer(string propertyId, string tttId, string buyer, string seller) returns (uint size)
    {
        // Actually calls itmap_impl.insert, auto-supplying the first parameter for us.
        insert(propertyId,tttId, buyer, seller);
    ...
    function removeTitleTransfer(string tttId) returns (uint size)
    {
     ...
     // Insert new TitleTransfer
    function updateTitleTransfer(string tttId, string buyer, string seller) returns (uint size)
    {
        update(tttId, buyer,  seller);
        // We can still access members of the struct - but we should take care not to mess with them.
        size = tttIdToTransfers.size;
    }
    ...
    function getTTTAddress(string tttId) constant returns (string propertyId, string buyer, string seller)
    {
        // We can still access members of the struct - but we should take care not to mess with them.
        (propertyId,buyer,seller) = get(tttId);
    }
	
	
    
    function getTTTInfo(uint index) constant returns (string propertyId,string tttId, string buyer, string seller)
    {
        (propertyId,tttId,buyer,seller) = iterate_get(index);
    }
    ...
    function insert(string propertyId, string tttId, string buyer, string seller) internal returns (bool replaced)
    {
        bytes32 key = sha3(tttId);
        uint keyIndex = tttIdToTransfers.data[key].keyIndex;
		tttIdToTransfers.data[key].propertyId = propertyId;
        tttIdToTransfers.data[key].tttId = tttId;
        tttIdToTransfers.data[key].buyer = buyer;
		tttIdToTransfers.data[key].seller = seller;
        if (keyIndex > 0)
            return true;
        else
        {
    ...
    function remove(string tttId) internal returns (bool success)
    {
	 ...
	 function update(string tttId,string buyer, string seller) internal returns (bool success)
    {
        bytes32 key = sha3(tttId);
        uint keyIndex = tttIdToTransfers.data[key].keyIndex;
        if (keyIndex == 0)
    ...
    function get(string tttId) internal  returns (string, string, string)
    {
        bytes32 key = sha3(tttId);
        return (tttIdToTransfers.data[key].propertyId, tttIdToTransfers.data[key].buyer, tttIdToTransfers.data[key].seller);
    }
    ...
    function iterate_get(uint keyIndex) internal returns (string propertyId, string tttId, string buyer, string seller)
    {
		KeyFlag flag = tttIdToTransfers.keys[keyIndex];
		if(!flag.deleted){
			bytes32 key = tttIdToTransfers.keys[keyIndex].key;
			propertyId = tttIdToTransfers.data[key].propertyId;
			tttId = tttIdToTransfers.data[key].tttId;
			buyer = tttIdToTransfers.data[key].buyer;
			seller = tttIdToTransfers.data[key].seller;
		}
    }
